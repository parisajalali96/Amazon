...
public class LoginMenuController {
    ...
    public void emptyCart() {
        ...
        if(shoppingCart == null || shoppingCart.getProductQuantity().isEmpty()) return;
        for (Product product : shoppingCart.getProductQuantity().keySet()) {
            shoppingCart.removeProduct(product, shoppingCart.getProductQuantity().get(product));

           // product.addInStock(shoppingCart.getProductQuantity().get(product));
            //shoppingCart.getProductQuantity().put(product, 0);
        }
    }
    ...
    public void removeFromCarts() {
        ...
        for (User user : App.getUsers()) {
            user.getShoppingCart().getProductQuantity().keySet().removeAll(products);
        }
    }
    public Result deleteAccount(String password, String rePassword) { //extra stuff
        if(App.getLoggedinUser() == null && App.getLoggedinStore() == null) {
            ...
            if(!rePassword.equals(password)) {
                return new Result(false, "Re-entered password is incorrect.");
            } else if(!storePasswordMatchesEmail(password, App.getLoggedinStore().getEmail())) {
                return new Result(false, "Password is incorrect.");
            }
            removeFromCarts();
