...
public class StoreMenuController {
    public Result addProduct(String name, String cost, String price, String description, String num) {
        ...
        int lastId = App.getNumberOfProducts() + 100;
    ...
    public Result showListOfProducts() {
        ...
        for (Product product : App.getLoggedinStore().getProducts()) {
            ...
            if (product.getDiscount() != null && product.getDiscount().getNumberApplied() < product.getDiscount().getQuantity()) { System.out.print("  **(On Sale! " + (product.getDiscount().getQuantity() - product.getDiscount().getNumberApplied()) + " units discounted)**");
            } else if (product.getInStock() == 0) {
                System.out.print("  (**Sold out!**)");
            }
            System.out.println();
            System.out.println("Name: " + product.getName());
            if (product.getDiscount() != null && product.getDiscount().getNumberApplied() < product.getDiscount().getQuantity()) {
                ...
                System.out.printf("Price: ~$%.1f~ → $%.1f (-%d%%)\n", product.getPrice(), discountedPrice, discountPercentage);
            } else {
