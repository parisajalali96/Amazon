...
public enum ProductCommands implements Command {
    ...
    ShowInformation("\\s*show information of\\s+-id\\s+(?<id>.+?)\\s*"),
    RateProduct("\\s*Rate product\\s+-r\\s+(?<rate>\\d+)\\s+(?:-m\\s+(?<message>.+?)\\s+)?-id\\s+(?<id>.+?)\\s*"),
    AddToCart("\\s*add to cart\\s+-product\\s+(?<productId>.+?)\\s+" +
