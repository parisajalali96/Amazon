...
public enum UserCommands implements Command {
    ...
    ShowOrderDetails("\\s*show order details\\s+-id\\s+(?<orderId>.+?)\\s*"),
    ...
    AddAddress("\\s*add address\\s+-country\\s+(?<country>.*?)\\s+" +
            ...
            "-postal\\s+(?<postal>.*?)"),
    Postal("\\d{10}"),
    DeleteAddress("\\s*delete address\\s+-id\\s+(?<addressId>.+?)\\s*"),
    ListMyAddresses("\\s*list my addresses\\s*"),
    AddCreditCard("\\s*add a credit card\\s+-number\\s(?<cardNumber>\\S+(\\s*\\S+)*)\\s+" +
    ...
    CVV("\\d{3,4}"),
    Value("\\d+(\\\\.\\\\d+)?"),
    ChargeCreditCard("\\s*Charge credit card\\s+-a\\s+(?<amount>.+?)\\s+" +
            "-id\\s+(?<cardId>.+?)"),
    CheckCreditCardBalance("\\s*Check credit card balance\\s+-id\\s+(?<cardId>.+?)\\s*"),
    ShowProductsinCart("\\s*show products in cart\\s*"),
    CheckOut("\\s*checkout\\s+-card\\s+(?<cardID>\\d+)\\s+" +
            "-address\\s+(?<addressId>.+?)\\s*"),
    RemoveFromCart("\\s*remove from cart\\s+-product\\s+(?<productId>.+?)\\s+" +
            "-quantity\\s+(?<amount>.+?)\\s*"),
    GoBack("\\s*go back\\s*");
