...
public enum UserCommands implements Command {
    ...
    AddAddress("\\s*add address\\s+-country\\s+(?<country>\\S+(\\s*\\S+)*)\\s+" +
            "-city\\s+(?<city>\\S+(\\s*\\S+)*)\\s+" +
            "-street\\s+(?<street>\\S+(\\s*\\S+)*)\\s+" +
            "-postal\\s+(?<postal>\\S+(\\s*\\S+)*)"),
    Postal("\\d{10}"),
    ...
    AddCreditCard("\\s*add a credit card\\s+-number\\s(?<cardNumber>\\S+(\\s*\\S+)*)\\s+" +
            ...
            "-cvv\\s+(?<cvv>\\S+(\\s*\\S+)*)\\s+" +
            "-initialValue\\s+(?<initialValue>\\S+(\\s*\\S+)*)\\s*"),
    CardNumber("\\d{16}"),
